<template>
  <b-container fluid>
    <b-col>
        ...
        <div class="wrapper">
          ...
          <template v-for="label in sorted_unique_labels">
            <template v-if="boxes_available.includes(label[0])">
              ...
              <b-button
                v-if="label[0].split(':')[1] == CL_operator"
                v-b-tooltip.hover
                variant="outline-info"
                :title="label[1]"
                size="sm"
                pill
                @click="updateMarkers(label[0].split(':')[0])"
              >
                {{ label[0].split(":")[0] + "*^" }}
              </b-button>
              <b-button
      ...
      <b-row align-h="center" class="my-1">
        <div v-if="isBusy === false" class="wrapper">
          <p class="text-muted">
            ({{ count_labels }} identified objects,
            {{ count_distinct_labels }} unique)
          </p>
          <hr />
          <p class="text-muted">
    ...
    <b-button type="button" @click="saveFile()">Download Data</b-button>
  </b-container>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      CL_operator: "custom_labels", // Custom Labels operator name
  ...
  computed: {
    ...
    sorted_unique_labels() {
      ...
      es_data.forEach(
        function(record) {
          // Because of Custom Labels, there could be a chance that label names will clash causing conflict and weird behaviour
          // This scheme of Name:Operator wil ensure uniqueness in the map and we will just split based on : when rendering in the UI
          const key = record.Name + ":" + record.Operator;
          unique_labels.set(
            key,
            unique_labels.get(key) ? unique_labels.get(key) + 1 : 1
          );
          if (
            ...
            this.saveBoxedLabel(key);
          }
        }.bind(this)
      );
      const sorted_unique_labels = new Map(
        [...unique_labels.entries()].slice().sort((a, b) => b[1] - a[1])
      );
      ...
      es_data.forEach(
        function(record, index) {
          if (record.Name === label) {
            ...
            } else if (record.Geometry && this.mediaType === "image") {
              // Seperately adding BB for custom labels. The BB info will be in the Geometry object.
              // Custom labels won't return an Instances array, it will return any duplicate labels as just another record
              const item = record.Geometry;
              const boxinfo = {
                instance: index,
                name: record.Name,
                confidence: (record.Confidence * 1).toFixed(2),
      ...
      if (this.mediaType === "image") {
        ...
        boxMap.forEach((i) => {
          ...
          if (drawMe) {
            ...
            ctx.fillText(
              drawMe.name + " (" + drawMe.confidence + "%)",
              drawMe.x + drawMe.width / 2,
              drawMe.y - 10
            );
            ctx.stroke();
          }
      ...
      this.canvasRefreshInterval = setInterval(
        function() {
          ...
          if (boxMap.has(player_timestamp)) {
            ...
            const instance_list = boxMap
              .get(player_timestamp)
    ...
    chartData() {
      ...
      function saveTimestamp(millisecond) {
        if (timeseries.has(millisecond)) {
          timeseries.set(millisecond, {
            x: millisecond,
            y: timeseries.get(millisecond).y + 1,
          });
        } else {
      ...
      const ordered_timeseries = new Map(
        [...timeseries.entries()].slice().sort((a, b) => a[0] - b[0])
      );
